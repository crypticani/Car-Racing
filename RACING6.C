/*
	Racing 6 changelog :
	1. No of tracks changed from 4 to 3
	2. frequency of obstacles increased to 200%
	3. Added Pause and Resume feature.
	4. bug at the select car screen slightly fixed for future debugging purpose.
	5. Continuous blinking of the screen fixed.
	6. Score is displayed after the game is over.
*/
#include<graphics.h>
#include<process.h>
#include<stdio.h>
		...
		for(j=0;j<500;j++)
		{
			...
			if(kbhit())
			{
				...
				if(ch==32)
				{
					...
					outtextxy(200,250,"Press Any Key to Resume");
					getch();
				}
				...
				if(ch==77&&i<345) //move right
				{
					if(i==344) //max value of x coordinate of car
					{
						i=343; // to remain on the track
					}
					i+=10;
				}
...
void track()
{
		...
		bar3d(410,0,410,getmaxy(),5,3);//right boundary
